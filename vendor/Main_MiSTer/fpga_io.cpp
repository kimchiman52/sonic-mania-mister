...
static int fpga_load_rbf_inner(const char *name, const char *cfg)
{
...
int fpga_load_rbf_no_restart(const char *name)
{
	return fpga_load_rbf_inner(name, nullptr);
}
...
int fpga_load_rbf(const char *name, const char *cfg, const char *xml)
{
	...
	int ret = fpga_load_rbf_inner(name, cfg);
