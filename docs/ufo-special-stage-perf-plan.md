...
**Deployed to MiSTer 2026-04-26.** Cross-compiled telemetry-flavor armhf via `tools/mister/build-game.sh --flavor telemetry` (Docker container `sonic-mania-mister-arm-build`); deployed to `192.168.1.188:/media/fat/games/sonic-mania/` (BuildID `2a6abfff5152f541b305bb10bd33cfcd02dace8d`). On-device 8s smoke test booted to the Sega/logos scene without crashing; clean SIGTERM exit. UFO Special Stage perf cannot be validated headlessly — requires a human at the controls to enter a giant ring. UFO5 (with Plasma) is the heaviest case to spot-check.

**Tooling friction worth fixing.** `tools/mister/deploy-to-mister.sh` whitelists `/media/fat/games/SonicMania/` (CamelCase), but the actual working install — where `Data.rsdk`, `SaveData.bin`, the wrapper-managed launch path, and prior deploys all live — is `/media/fat/games/sonic-mania/` (lowercase, hyphenated). The deploy script silently writes to the abandoned CamelCase path. Worked around for this deploy by manual `rsync`; the script's whitelist should be updated (or replaced with the lowercase path) so future deploys don't need the workaround.
